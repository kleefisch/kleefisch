...
export function Contact() {
  ...
  return (
    <section
      id="contact"
      className="relative flex min-h-[100dvh] flex-col overflow-hidden scroll-mt-16"
    >
      {/* Dot grid texture */}
      <div className="absolute inset-0 bg-dot-grid pointer-events-none opacity-60" />
      {/* Violet aurora — slow drift bottom-center */}
      <motion.div
        animate={{ x: [-160, 160, -160], y: [100, -80, 100] }}
        transition={{ duration: 30, repeat: Infinity, ease: "easeInOut" }}
        className="absolute bottom-0 left-1/4 w-[900px] h-[700px] rounded-full bg-accent-violet/18 blur-[140px] mix-blend-screen pointer-events-none"
      />
      {/* Cyan aurora — slow drift top-right to bottom-left */}
      <motion.div
        animate={{ x: [160, -180, 160], y: [-80, 140, -80] }}
      ...
      <div className="relative z-10 flex flex-1 flex-col">
                  ...
                  {status === "success" ? (
                    ...
                    >
                      ...
                      <button
                        type="submit"
                        disabled={isPending}
                        className="group relative h-12 w-full sm:w-auto px-8 rounded-md font-medium text-foreground hover:shadow-[0_0_20px_rgba(6,182,212,0.3)] disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"
                        style={{
                          background:
        ...
        <div className="relative z-10 mt-auto shrink-0 bg-background/80 backdrop-blur-sm">
          <Footer />
        </div>
      </div>
    </section>
