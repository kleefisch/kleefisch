...
import { deleteMessageAction, toggleMessageReadAction, archiveMessageAction } from "@/app/actions/contact";
...
export default async function AdminDashboardPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const unreadMessagesCount = allMessages.filter((m) => !m.read && !m.archived).length;
  
  const messages = allMessages.filter((m) => messagesFilter === "archived" ? m.archived : !m.archived);
      ...
      <main className="flex-1 container mx-auto px-4 max-w-7xl py-10 space-y-8 relative z-10 w-full">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex flex-col gap-3 rounded-2xl border border-white/5 bg-white/[0.02] p-6 relative overflow-hidden group">
            ...
            <p className="text-3xl font-bold font-mono tracking-tight text-white z-10">{allMessages.length}</p>
          </div>
        </section>
        ...
        <div className="flex items-center gap-6 border-b border-white/5 mt-8 px-1">
          ...
          >
            ...
            {unreadMessagesCount > 0 && (
              <span className="bg-accent-violet text-white text-[10px] px-1.5 py-0.5 rounded-full leading-none">{unreadMessagesCount}</span>
            )}
          </Link>
        </div>
              ...
              <div className="w-full">
                {messages.length === 0 ? (
                  <div className="rounded-3xl border border-white/5 bg-white/[0.02] p-12 text-center text-neutral-500 flex flex-col items-center">
                    ...
                    Nenhuma mensagem {messagesFilter === 'archived' ? 'arquivada' : 'na caixa de entrada'}.
                  </div>
                ) : (
                        ...
                        <div className="flex flex-col gap-5 relative z-10 w-full mb-6">
                          ...
                          <div className="flex justify-between items-start border-b border-white/5 pb-5">
                            <div className="flex gap-4 items-center w-full">
                              <div className={`h-12 w-12 shrink-0 border rounded-[14px] flex items-center justify-center font-bold text-xl ${msg.read ? 'bg-white/5 border-white/10 text-white/50' : 'bg-accent-violet/10 border-accent-violet/30 text-accent-violet'}`}>
                                {msg.name.charAt(0).toUpperCase()}
                              </div>
                              <div className="flex flex-col w-full gap-0.5">
                                <div className="flex items-center justify-between w-full">
                                  <span className="text-white text-[15px] font-semibold">{msg.name}</span>
                          ...
                          {/* Email Body */}
                          <div className={`text-[14px] leading-relaxed whitespace-pre-wrap ${msg.read ? 'text-neutral-400' : 'text-neutral-200 font-medium'}`}>
                            {msg.message}
                          </div>
                        </div>
                          ...
                          <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
                            ...
                            <form action={async () => {
                              ...
                              <button className="flex items-center justify-center gap-1.5 px-3 py-2 text-[11px] font-bold tracking-wider uppercase rounded-md bg-white/5 text-neutral-300 hover:bg-white/10 transition-colors w-full sm:w-auto">
                                ...
                                <span className="hidden xl:inline">{msg.read ? "Marcar Não Lido" : "Marcar Lida"}</span>
                              </button>
                            </form>
                            ...
                            <form action={async () => {
                              ...
                              <button className="flex items-center justify-center gap-1.5 px-3 py-2 text-[11px] font-bold tracking-wider uppercase rounded-md bg-white/5 text-neutral-300 hover:bg-white/10 transition-colors w-full sm:w-auto">
                                ...
                                <span className="hidden xl:inline">{msg.archived ? "Desarquivar" : "Arquivar"}</span>
                              </button>
                            </form>
                            ...
                            <form action={async () => {
                              ...
                              <button className="flex items-center justify-center p-2 text-neutral-400 hover:bg-red-500/10 hover:text-red-400 rounded-md transition-colors opacity-0 group-hover:opacity-100">
